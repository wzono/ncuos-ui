<template>
  <div class="us-datetime" :class="{'us-datetime__clearable': clearable && text && !disabled}">
    ...
    <a class="us-datetime-close" @click.stop="cls"></a>
    <transition name="us-datetime-anim">
      <div class="us-datetime-popup" :class="[popupClass, {'us-datetime-inline' : type === 'inline'}]" tabindex="-1" v-if="show || type === 'inline'">
        <template>
          <calendar v-model="dates[0]" :precision="precision"></calendar>
        </template>
        <div v-if="showButtons" class="us-datetime__buttons">
          <button @click.prevent.stop="cancel" class="us-datetime__button-cancel">{{this.local.cancelTip}}</button>
          <button @click.prevent.stop="submit" class="us-datetime__button-select">{{this.local.submitTip}}</button>
        </div>
      </div>
    </transition>
  </div>
