  ...
  <div :class="`${pre}`">
    ...
    <div :class="`${pre}-body`">
      ...
      <div :class="`${pre}-months`" v-show="showMonths">
        <a v-for="(i, j) in local.months"
           @click="is($event) && (showMonths = (precision === 'M'), month = j, (precision === 'M' && ok()))"
           :class="[status(year, j, day, hour, minute, second, 'YYYYMM')]" :key="j">
          {{i}}
        </a>
      </div>
      <div :class="`${pre}-years`" v-show="showYears">
        <a v-for="(i, j) in years"
           @click="is($event) && (showYears = (precision === 'Y'), year = i, (precision === 'Y' && ok()))"
    ...
    <div :class="`${pre}-foot`" v-if="precision === 'H'">
      <div :class="`${pre}-hour`">
        <a :title="local.hourTip"
